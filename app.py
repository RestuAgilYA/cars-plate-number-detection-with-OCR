...
def detect_platnomer(filepath):
    ...
    results = model.predict(filepath, save=True, conf=0.3, project="static", name="deteksi", save_crop=True)
    return results
...
def upload():
    ...
    if file and allowed_file(file.filename):
        ...
        return render_template('hasildeteksi.html', filename=filename, is_image=is_image, result_file=result_file_path, ocr_results=ocr_results)
    else:
